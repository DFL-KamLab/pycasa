...
def median(
    ...
    *,
    ...
    if verbose:
        print(f"Running normalization median on frames (overwrite={overwrite})...")
    ...
    for frame in _progress_bar(
        original_video,
        total=frame_count,
        desc="Normalization median",
        unit="frame",
        leave=True,
        enabled=show_progress,
    ):
        normalized_frames.append(
            _framewise_minmax(frame.astype(np.float32) - np.median(frame.astype(np.float32)))
        )
